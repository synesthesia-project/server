...
const trackPath = (dataDir: string, id: string) => path.join(dataDir, CUE_FILES_PATH, id);
const trackRevisionsPath = (dataDir: string, id: string) => path.join(dataDir, CUE_FILES_PATH, id, 'revisions');
const trackCurrentRevisionPath = (dataDir: string, id: string) => path.join(trackPath(dataDir, id), 'current.scue');
const trackRevisionPath = (dataDir: string, id: string, revision: number | string) =>
...
export class Storage {
  ...
  public async saveFile(trackId: string, file: CueFile): Promise<void> {
    const pathname = trackRevisionPath(this.dataDir, trackId, new Date().getTime());
  ...
  public async getRevisions(trackId: string): Promise<string[]> {
    const list = await fs.readdir(trackRevisionsPath(this.dataDir, trackId))
      .catch((err: Error) => {
        if (err.message.startsWith('ENOENT'))
          return [] as string[];
        throw err;
    ...
    console.log(list);
    return list.filter(name => name.endsWith('.scue')).map(name => name.substr(0, name.length - 5));
  }

  /**
   * Get a specific revision of a track.
   */
  public async getRevision(trackId: string, revisionName: string): Promise<CueFile> {
